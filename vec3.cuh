...
class vec3 {
  public:
    ...
    __device__ int& operator[](int i) { return e[i]; }
    ...
    __device__ vec3& operator*=(int t) {
        e[0] *= t;
        e[1] *= t;
        e[2] *= t;
        return *this;
...
__device__ inline vec3 operator*(int t, const vec3& v) {
    return vec3(t*v.e[0], t*v.e[1], t*v.e[2]);
}

__device__ inline vec3 operator*(const vec3& v, int t) {
    return t * v;
}
...
__device__ inline int dot(const vec3& u, const vec3& v) {
    return u.e[0] * v.e[0]
         + u.e[1] * v.e[1]
         + u.e[2] * v.e[2];
}
